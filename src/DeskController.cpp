...
void DeskController::loop() {
  ...
  while (link.available() > 0) {
    ...
    if (decoder_.feed(b)) {
      ...
      child_locked_ = false;
      if (decoder_.height() != last_seen_height_) {
        last_seen_height_ = decoder_.height();
        last_height_change_at_ = at;
      }
      // With the alarm armed, the display only shows the height during (and
      // briefly after) movement before returning to "=XX" — so a height that
      // has stayed static this long, with no "=" frame for equally long, is
      // positive evidence the alarm was switched off (e.g. from the handset).
      if (alarm_on_ && at - last_alarm_frame_at_ > cfg::kAlarmClearTimeout &&
          at - last_height_change_at_ > cfg::kAlarmClearTimeout) {
        alarm_on_ = false;
      }
      if (height_cb_) height_cb_(decoder_.height());
    }
    if (decoder_.isLockDisplay()) child_locked_ = true;
    if (decoder_.isAlarmDisplay()) {
      alarm_on_ = true;
      last_alarm_frame_at_ = millis();
    }
  }

  const uint32_t now = millis();

  // The display never sleeps while the alarm is armed — the "=XX" blink keeps
  // it streaming — so a fully dark display (no frames of any kind this long)
  // also means the alarm is off.
  if (alarm_on_ && now - last_alarm_frame_at_ > cfg::kAlarmClearTimeout &&
      now - last_height_at_ > cfg::kAlarmClearTimeout) {
    alarm_on_ = false;
  }
...
void DeskController::childLock() {
  planner_.holdCommand(desk_cmd::ChildLock, millis(), cfg::kChildLockHold);
}

void DeskController::alarmOff() {
  planner_.holdCommand(desk_cmd::Alarm, millis(), cfg::kAlarmOffHold);
}
